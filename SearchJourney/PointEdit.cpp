...
void PointEdit::addPoint(QgsMapMouseEvent* e)
{
    ...
    if (mPointType == "地图点") {
        ...
        // 通过 GeometryEditTool 添加点要素并支持撤销重做
        if (!mGeometryEditTool) {
            QMessageBox::warning(nullptr, "错误", "编辑工具未初始化！");
            return;
        }
...
// 设置几何体重做编辑工具
void PointEdit::setGeometryEditTool(GeometryEditTool* tool)
{
	mGeometryEditTool = tool;
}